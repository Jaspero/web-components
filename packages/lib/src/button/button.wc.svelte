...
<button {disabled} {type} class="jp-button jp-{variant}-button {color}" class:loading on:click>
  <slot />
</button>
...
<style>
  ...
  .jp-button:not(.jp-basic-button):not(.jp-raised-button):not(.jp-stroked-button):not(
      .jp-flat-button
    ) {
    overflow: hidden;
    border-radius: 50%;
