...
<script lang="ts">
  ...
  $: {
    ...
    attachedInternals.setFormValue(url);
    dispatch('value', { value: url });
  }
...
<div class="flex">
  <input type="text" {name} {id} bind:value={url} disabled={isLocal}>
  ...
  <div class="preview">
    <img src={isLocal ? img : url} width="100px" alt="preview" hidden={!url && !img}>
  </div>
  ...
  <button hidden={!isLocal} on:click|preventDefault={() => {isLocal = false; img = ''; url = ''}}>
